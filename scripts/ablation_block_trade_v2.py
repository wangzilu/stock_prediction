"""12-split rolling ablation for block_trade v2 discount-rate factors against 174-base."""
...
# ---------- load block_trade v2 factors ----------
logger.info("Loading block_trade_v2.parquet...")
df = pd.read_parquet(str(DATA_DIR / "block_trade_v2.parquet"))
df['date'] = pd.to_datetime(df['date'])
df['qlib_code'] = df['qlib_code'].str.upper()

factor_cols = ['bt_discount', 'bt_discount_5d_avg', 'bt_has_recent',
...
# Fill bt_has_recent NaN -> 0 (no block trade = 0)
if 'bt_has_recent' in merged.columns:
    merged['bt_has_recent'] = merged['bt_has_recent'].fillna(0)
...
for split_idx in range(n_splits):
    ...
    results["block_trade_v2"].append(delta)
...
# ---------- save ----------
out_path = DATA_DIR / "phase4" / "block_trade_v2_ablation.json"
out_path.parent.mkdir(parents=True, exist_ok=True)
payload = {
    "factor_group": "block_trade_v2_discount",
